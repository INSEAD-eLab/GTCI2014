...
datafile <- loadWorkbook(paste(local_directory, "/data/2014 05 19 JRC submission based on GTCI 2014 framework.xlsx", sep=""))

Level <- 3 # 1 or 2 or 3

if(Level == 1){
  ProjectData <- readWorksheet(datafile, sheet="Normalized", region="B5:CG112", header=T)
  
  number_of_variables=c(5,7,5,10,7,7,4,7,7,6,4,3,6,4)
  ...
  ProjectData <- readWorksheet(datafile, sheet="Calculation", region="B1:Y108", header=T)
