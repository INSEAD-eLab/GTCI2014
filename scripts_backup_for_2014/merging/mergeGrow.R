...
head (pupil.teacher.ratio.secondary)
...
colnames(pupil.teacher.ratio.secondary)[3] <- "Pupil-teacher ratio (GTCI2013)"

## merge
merge31 <- merge(Technical.vocational.enrolment[, c(3,4)], Tertiary.enrolment[, c(3,4)], by="ISO3", all=TRUE, sort=FALSE)
merge31 <- merge (merge31,science.engineering.ratios[, c(6:7)], by="ISO3", all=TRUE, sort=FALSE)
...
merge31 <- merge (merge31,pupil.teacher.ratio.secondary[, c(3,4)], by="ISO3", all=TRUE, sort=FALSE)
...
head (journals.WB.scaled)
head (Willingness.to.delegate.authority)
head (voice.to.officials)
...
colnames(voice.to.officials)[3] <- "Voicing concern to officials"
...
merge33 <- merge(merge33, voice.to.officials[, c(1,3)], by="ISO3", all=TRUE, sort=FALSE)
